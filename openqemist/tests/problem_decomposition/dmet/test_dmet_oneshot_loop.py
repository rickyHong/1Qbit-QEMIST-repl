...
class TestDMETloop(unittest.TestCase):
    ...
    def test_dmet_functions(self):
        ...
        onerdm_low_ref = np.loadtxt(get_file_path_stub() + 'test_dmet_oneshot_loop_low_rdm.txt')
        for index, value_ref in np.ndenumerate(onerdm_low_ref):
            self.assertAlmostEqual(value_ref, onerdm_low[index], msg='Low-level RMD error at index ' + str(index), delta=1e-6)
        ...
        onerdm_high_ref = np.loadtxt(get_file_path_stub() + 'test_dmet_oneshot_loop_core_rdm.txt')
